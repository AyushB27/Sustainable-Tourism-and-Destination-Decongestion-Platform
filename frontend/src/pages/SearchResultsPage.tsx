    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight mt-0.5">
              {query ? `${t.srResultsForPrefix} "${query}"` : t.srAllResultsTitle}
            </h1>
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              {t.srFoundPrefix} {matches.length} {t.srFoundSuffix}
            </p>
          </div>
          <button
          ...
          >
            {t.srSwitchToDiscover}
          </button>
        </div>

        <div className="max-w-2xl">
          <GlobalSearchBox variant="hero" placeholder={t.srSearchPlaceholder} />
        </div>
      </div>
      ...
      {matches.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center space-y-4 shadow-sm">
          ...
          <h2 className="text-lg font-bold text-slate-900">{t.srNoMatchTitlePrefix} &ldquo;{query}&rdquo;</h2>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span>{t.srDestinationsHeading} ({spotMatches.length})</span>
                </h2>
                      ...
                      <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                        ...
                        <div className="pt-2 border-t border-slate-100 flex items-center justify-between">
                          ...
                          <span className="text-xs font-bold text-gov-navy flex items-center gap-1 group-hover:translate-x-0.5 transition">
                            {t.srViewSpot} <ArrowRight className="w-3.5 h-3.5" />
                          </span>
                        </div>
                      </div>
                    </Link>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span>{t.srDistrictsHeading} ({districtMatches.length})</span>
                </h2>
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
                  ...
                  <span>{t.srStatesHeading} ({stateMatches.length})</span>
                </h2>
